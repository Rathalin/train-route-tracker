	...
	<div class="mx-4 mt-4 flow-root">
		<h1 class="h1 mt-4 mb-10 flex flex-row justify-between items-end gap-2 flex-wrap">
			...
			<div class="flex flex-row items-center flew-wrap">
				<a href="/train-routes/{data.route.shortName}/edit" class="btn variant-ghost-primary">
					<EditIcon />
					<span>{$_('route.train-routes.slug.button.edit-route.label')}</span>
				</a>
			</div>
		</h1>
		...
		<div class="table-container">
			<table class="table table-hover mb-10">
				<thead>
					<tr>
						<th class="w-10">{$_('route.train-routes.slug.table.progress')}</th>
						<th class="table-cell-fit">{$_('route.train-routes.slug.table.kilometer')}</th>
						<th class="table-cell-fit"></th>
						<th>{$_('route.train-routes.slug.table.description')}</th>
						<th class="w-1/2">{$_('route.train-routes.slug.table.notes')}</th>
					</tr>
				</thead>
...
{:else}
	<h1 class="h1 mt-10">{$_('route.train-routes.slug.error.route-not-found.message')}</h1>
	<div class="mt-6">
		<HomeLinkButton />
	</div>
{/if}
